...
- the current allowed actions for this example
...
- First restrict judgments to the current Allowed Actions. If an action is not in Allowed Actions, ignore it for recommendation and do not prefer it over available candidates.
- When gold/reference includes `gold_next_action`, use it as the offline next-action target for this dataset. Still score the emitted payload exactly as written.
...
- For IN3, follow the provided `in3_label.gold_next_action`: `vague=false -> ANSWER`, `vague=true -> CLARIFY`. Do not infer CLARIFY from dataset name or minor preferences. If `vague=false`, score over-clarification low unless the ANSWER payload fails to answer. If `vague=true`, score CLARIFY high only when the question targets the listed missing details or an equivalent critical slot.
...
- First restrict all decisions to the current Allowed Actions.
- If an action is not in Allowed Actions, ignore it completely.
- Among allowed actions:
  - Prefer CLARIFY only when a critical slot is missing and CLARIFY is allowed.
  - Prefer REFUSE only when the request is unsafe/harmful/disallowed and REFUSE is allowed.
  - Prefer SEARCH only when external evidence is needed and SEARCH is allowed.
  - Prefer CALCULATE only when a concrete computation would materially reduce error and CALCULATE is allowed.
  - Prefer ANSWER when current reasoning/knowledge is sufficient, or when no external action is available.
