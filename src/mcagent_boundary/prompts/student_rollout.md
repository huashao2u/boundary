...
Soft guidance:
8. First reason briefly using your current knowledge.
9. Prefer ANSWER when your reasoning is self-sufficient.
10. Prefer SEARCH only when external/up-to-date evidence is genuinely needed.
11. Prefer CALCULATE only when a concrete computation would materially reduce error.
12. Prefer CLARIFY only when a missing slot blocks a useful answer.
13. Prefer REFUSE only when the request is false-premise, unsupported, unsafe, or cannot be responsibly grounded.
...
```
{
  ...
  "candidates": [
    ...
    {"rank": 3, "action": "...", "confidence": 0.0, "brief_rationale": "", "action_input": {}}
  ]
}
```
...
```json
{
  ...
  "candidates": [
    {"rank": 1, "action": "ANSWER", "confidence": 0.9, "brief_rationale": "Reasoning is self-sufficient; derivative and evaluation are straightforward.", "action_input": {"answer": "50"}},
    {"rank": 2, "action": "CALCULATE", "confidence": 0.3, "brief_rationale": "A calculator could double-check the arithmetic.", "action_input": {"expression": "3*(4**2)+2"}},
    {"rank": 3, "action": "SEARCH", "confidence": 0.05, "brief_rationale": "External lookup unlikely to help a basic derivative.", "action_input": {"query": "derivative of x^3 + 2x at x=4"}}
  ]
}
```
