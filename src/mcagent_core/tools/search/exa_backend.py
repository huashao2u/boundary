...
class ExaBackend:
    ...
    def __init__(self, config: dict[str, Any], top_k: int = 3, phase: str = "eval"):
        ...
        self.top_k = top_k
        self.phase = phase
        self.endpoint = str(config.get("exa_base_url", "https://api.exa.ai/search"))
        self.api_key_env = str(config.get("exa_api_key_env", "EXA_API_KEY"))
    ...
    def run(self, action_input: dict[str, Any], sample, history: list[dict[str, Any]]) -> tuple[dict[str, Any], bool, dict[str, Any]]:
        ...
        response = http_post_json(
            ...
            {
                "query": query,
                "type": str(self.config.get("exa_search_type", "auto")),
            ...
            rows.append(
                {
                    ...
                    "content": item.get("text") or item.get("highlights"),
                    "url": item.get("url"),
        ...
        return build_online_observation(
            ...
            query=query,
            rows=rows,
            request_metadata={"request_id": response.get("requestId"), "search_type": response.get("searchType")},
        )
